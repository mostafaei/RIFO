...
saveDir='projects/rifo/plots/rifo_evaluation/fairness/data_mining_workload/'
file = "projects/rifo/plots/rifo_evaluation/fairness/data_mining_workload/pFabric_less_100KB_99th_fct_ms.dat"
r = open(file, 'r')
lines = r.readlines()
for i, line in enumerate(lines):
    ...
    if len(l)>6:
        ...
        AFQ_99FCT.append(float(l[3]))
        SPPIFO_99FCT.append(float(l[4]))
        PIFO_99FCT.append(float(l[5]))
        AIFO_99FCT.append(float(l[6]))
        RIFO_99FCT.append(float(l[7]))
    
file = "projects/rifo/plots/rifo_evaluation/fairness/data_mining_workload/pFabric_less_100KB_mean_fct_ms.dat"
r = open(file, 'r')
lines = r.readlines()
for i, line in enumerate(lines):
    l=re.findall("[-+]?[.]?[\d]+(?:,\d\d\d)*[\.]?\d*(?:[eE][-+]?\d+)?", line)
    if len(l)>6:
        ...
        AFQ_mean_small.append(float(l[3]))
        SPPIFO_mean_small.append(float(l[4]))
        PIFO_mean_small.append(float(l[5]))
        AIFO_mean_small.append(float(l[6]))
        RIFO_mean_small.append(float(l[7]))
    

file = "projects/rifo/plots/rifo_evaluation/fairness/data_mining_workload/pFabric_geq_1MB_mean_fct_ms.dat"
r = open(file, 'r')
lines = r.readlines()
for i, line in enumerate(lines):
    l=re.findall("[-+]?[.]?[\d]+(?:,\d\d\d)*[\.]?\d*(?:[eE][-+]?\d+)?", line)
    if len(l)>6:
        ...
        AFQ_large.append(float(l[3]))
        SPPIFO_large.append(float(l[4]))
        PIFO_large.append(float(l[5]))
        AIFO_large.append(float(l[6]))
        RIFO_large.append(float(l[7]))
...
ax.plot((AFQ_mean_small),label='AFQ' ,color=AFQ_color,linestyle='-', mfc='tab:brown',marker='>', markersize=3,lw=1 ,  mew=0.65)
...
fig.tight_layout() 
name='avgFCT-datamining-wfq'
plt.savefig(saveDir+'%s.pdf'%name,format="pdf", bbox_inches='tight', pad_inches=0.05)
plt.close()
#******************************** 99FCT small*************************************
set_paper_rcs()
...
ax.plot((RIFO_99FCT),label='RIFO' ,color=RIFO_color,linestyle='-', mfc='tab:red',marker='d', markersize=3,lw=1,  mew=0.65)
ax.plot((AIFO_99FCT),label='AIFO' ,color=AIFO_color,linestyle='-', mfc='tab:blue',marker='s', markersize=3,lw=1 ,  mew=0.65)
ax.plot((SPPIFO_99FCT),label='SP-PIFO' ,color=SPPIFO_color,linestyle='-', mfc='tab:orange',marker='o', markersize=3,lw=1 ,  mew=0.65)
ax.plot((PIFO_99FCT),label='PIFO' ,color=PIFO_color,linestyle='-', mfc='tab:green',marker='X', markersize=3,lw=1 ,  mew=0.65)
ax.plot((DCTCP_99FCT),label='DCTCP' ,color=DCTCP_color,linestyle='-', mfc='tab:purple',marker='^', markersize=3,lw=1 ,  mew=0.65)
ax.plot((TCP_99FCT),label='TCP' ,color=TCP_color,linestyle='-', mfc='tab:olive',marker='v', markersize=3,lw=1 ,  mew=0.65)
ax.plot((AFQ_99FCT),label='AFQ' ,color=AFQ_color,linestyle='-', mfc='tab:brown',marker='>', markersize=3,lw=1 ,  mew=0.65)
...
ax.plot((AFQ_large),label='AFQ' ,color=AFQ_color,linestyle='-', mfc='tab:brown',marker='>', markersize=3,lw=1 ,  mew=0.65)
...
ax.grid(color='black', ls = '--',dashes=(5, 15), lw = 0.2,alpha=1)
fig.tight_layout() 
name='avgFCTLarge-datamining-wfq'
plt.savefig(saveDir+'%s.pdf'%name,format="pdf", bbox_inches='tight', pad_inches=0.05)
plt.close()
