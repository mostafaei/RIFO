...
saveDir='projects/rifo/plots/rifo_evaluation/pFabric/web_search_workload_pareto/'
file = "projects/rifo/plots/rifo_evaluation/pFabric/web_search_workload_pareto/pFabric_less_100KB_99th_fct_ms.dat"
r = open(file, 'r')
lines = r.readlines()
for i, line in enumerate(lines):
    ...
    if len(l)>1:
        ...
        MIFO_99FCT.append(float(l[6]))
        RIFO_99FCT.append(float(l[7]))
    
file = "projects/rifo/plots/rifo_evaluation/pFabric/web_search_workload_pareto/pFabric_less_100KB_mean_fct_ms.dat"
r = open(file, 'r')
lines = r.readlines()
for i, line in enumerate(lines):
    l=re.findall("[-+]?[.]?[\d]+(?:,\d\d\d)*[\.]?\d*(?:[eE][-+]?\d+)?", line)
    if len(l)>1:
        ...
        MIFO_mean_small.append(float(l[6]))
        RIFO_mean_small.append(float(l[7]))
    

file = "projects/rifo/plots/rifo_evaluation/pFabric/web_search_workload_pareto/pFabric_geq_1MB_mean_fct_ms.dat"
r = open(file, 'r')
lines = r.readlines()
for i, line in enumerate(lines):
    l=re.findall("[-+]?[.]?[\d]+(?:,\d\d\d)*[\.]?\d*(?:[eE][-+]?\d+)?", line)
...
#ax.plot((MIFO_mean_small),label='MIFO' ,color=MIFO_color,linestyle='-', mfc='black',marker='d', markersize=3,lw=1 ,  mew=0.65)
...
#ax.plot((MIFO_99FCT),label='MIFO' ,color=MIFO_color,linestyle='-', mfc='black',marker='d', markersize=3,lw=1,  mew=0.65)
...
#ax.plot((MIFO_large),label='MIFO' ,color=MIFO_color,linestyle='-', mfc='black',marker='d', markersize=3,lw=1 ,  mew=0.65)
...
ax.grid(color='black', ls = '--',dashes=(5, 15), lw = 0.2,alpha=1)
fig.tight_layout() 
name='avgFCTLarge-websearch-pareto'
plt.savefig(saveDir+'%s.pdf'%name,format="pdf", bbox_inches='tight', pad_inches=0.05)
plt.close()
