...
for i, line in enumerate(lines):
    ...
    if len(l)>1:
        B50_all_Throughput.append(float(l[1]))
        B100_all_Throughput.append(float(l[2]))
        B500_all_Throughput.append(float(l[3]))
...
for i, line in enumerate(lines):
    l=re.findall("[-+]?[.]?[\d]+(?:,\d\d\d)*[\.]?\d*(?:[eE][-+]?\d+)?", line)
    if len(l)>1:
        B50_mean_small.append(float(l[1]))
        B100_mean_small.append(float(l[2]))
        B500_mean_small.append(float(l[3]))
...
for i, line in enumerate(lines):
    l=re.findall("[-+]?[.]?[\d]+(?:,\d\d\d)*[\.]?\d*(?:[eE][-+]?\d+)?", line)
    if len(l)>1:
        B50_large.append(float(l[1]))
        B100_large.append(float(l[2]))
        B500_large.append(float(l[3]))
...
ax.plot((B500_mean_small),label='T=B=500' ,color=B500_color,linestyle='-', mfc='tab:blue',marker='s', markersize=3,lw=1 ,  mew=0.65)
ax.plot((B100_mean_small),label='T=B=100' ,color=B100_color,linestyle='-', mfc='tab:orange',marker='o', markersize=3,lw=1 ,  mew=0.65)
ax.plot((B50_mean_small),label='T=B=50' ,color=B50_color,linestyle='-', mfc='tab:green',marker='X', markersize=3,lw=1 ,  mew=0.65)
...
fig.tight_layout() 
name='avgFCT-small-websearch-qLen-T=B'
...
ax.plot((B500_all_Throughput),label='RIFO T=B=500' ,color=B500_color,linestyle='--', mfc='tab:blue',marker='s', markersize=3,lw=1 ,  mew=0.65)
ax.plot((B100_all_Throughput),label='RIFO T=B=100' ,color=B100_color,linestyle='--', mfc='tab:orange',marker='o', markersize=3,lw=1 ,  mew=0.65)
ax.plot((B50_all_Throughput),label='RIFO T=B=50' ,color=B50_color,linestyle='--', mfc='tab:green',marker='X', markersize=3,lw=1 ,  mew=0.65)
...
fig.tight_layout() 
name='avgThroughput-all-websearch-qLen-T=B-RIFO'
plt.savefig('%s.pdf'%name,format="pdf", bbox_inches='tight', pad_inches=0.05)
plt.close()
...
set_paper_rcs_Erdos()
fig,ax = plt.subplots(nrows=1, ncols=1)
ax.plot((B500_large),label='T=B=500' ,color=B500_color,linestyle='-', mfc='tab:blue',marker='s', markersize=3,lw=1 ,  mew=0.65)
ax.plot((B100_large),label='T=B=100' ,color=B100_color,linestyle='-', mfc='tab:orange',marker='o', markersize=3,lw=1 ,  mew=0.65)
ax.plot((B50_large),label='T=B=50' ,color=B50_color,linestyle='-', mfc='tab:green',marker='X', markersize=3,lw=1 ,  mew=0.65)
...
ax.grid(color='black', ls = '--',dashes=(5, 15), lw = 0.2,alpha=1)
fig.tight_layout() 
name='avgFCT-large-websearch-qLen-T=B'
plt.savefig('%s.pdf'%name,format="pdf", bbox_inches='tight', pad_inches=0.05)
plt.close()
